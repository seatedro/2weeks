...
const ENCRYPTED_MESSAGES = [
  ...
  },
  {
    ...
    message: 'Access Code: 42\ndebug_mode.enabled = true\nPress F12 to pierce the veil',
    type: 'info'
  },
  {
    id: 'matrix',
    title: '>> NEURAL PATHWAY UNLOCKED <<',
    message: 'Click the red glyphs\nFollow the white rabbit\nWatch for ¥€§∆πΩμ∑∫√',
    type: 'error'
  }
];
...
const MessageBox = ({ message, onClose }) => {
  ...
  return (
    <div className={`fixed bottom-8 right-8 transition-all duration-500 transform ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}>
      <div className="bg-black border border-green-400 p-4 rounded-lg shadow-lg max-w-md">
        <div className="flex items-center gap-2 mb-2">
          <AlertTriangle className="text-yellow-400" size={20} />
          <h3 className="text-green-400 font-bold font-mono">{message.title}</h3>
        </div>
        <pre className="text-green-400 font-mono text-sm whitespace-pre-wrap">{message.message}</pre>
      </div>
    </div>
  );
};
...
const useMatrixRain = () => {
  ...
  useEffect(() => {
    ...
    const handleClick = (e) => {
      ...
      if (glitchCharsRef.current[column] &&
        Math.abs(y - e.clientY) < fontSize * 2) {
        // Show message
        const randomMessage = ENCRYPTED_MESSAGES[Math.floor(Math.random() * ENCRYPTED_MESSAGES.length)];
